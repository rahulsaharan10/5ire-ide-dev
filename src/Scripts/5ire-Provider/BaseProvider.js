const safe_event_emitter = require("./safe-event-emitter/index");
 ...
 */
export class BaseProvider extends safe_event_emitter.default {
    /**
    ...
    constructor({ logger = console, maxEventListeners = 100, rpcMiddleware = [], } = {}) {
        ...
        console.log("rpc middleware count: ", rpcMiddleware);
        ...
        // the server.
        const rpcEngine = new json_rpc_engine.JsonRpcEngine();
    ...
    async request(args) {
        if (!args || typeof args !== 'object' || Array.isArray(args)) {
            throw eth_rpc_errors.ethErrors.rpc.invalidRequest({
                message: messages.default.errors.invalidRequestArgs(),
                data: args,
            });
        }
        ...
        if (typeof method !== 'string' || method.length === 0) {
            throw eth_rpc_errors.ethErrors.rpc.invalidRequest({
                message: messages.default.errors.invalidRequestMethod(),
                data: args,
            });
        }
        if (params !== undefined &&
            ...
            throw eth_rpc_errors.ethErrors.rpc.invalidRequest({
                message: messages.default.errors.invalidRequestParams(),
                data: args,
            });
        }
        return new Promise((resolve, reject) => {
            this._rpcRequest({ method, params }, utils.getRpcPromiseCallback(resolve, reject));
        });
    }
    //====================
    // Private Methods
    ...
    _handleConnect(chainId) {
        if (!this._state.isConnected) {
            ...
            this._log.debug(messages.default.info.connected(chainId));
        }
    }
    /**
    ...
    _handleDisconnect(isRecoverable, errorMessage) {
        if (this._state.isConnected ||
            ...
            let error;
            if (isRecoverable) {
                error = new eth_rpc_errors.EthereumRpcError(1013, // Try again later
                errorMessage || messages.default.errors.disconnected());
                this._log.debug(error);
            }
            else {
                error = new eth_rpc_errors.EthereumRpcError(1011, // Internal error
                errorMessage || messages.default.errors.permanentlyDisconnected());
    ...
    _handleChainChanged({ chainId, } = {}) {
        if (!utils.isValidChainId(chainId)) {
            this._log.error(messages.default.errors.invalidNetworkParams(), { chainId });
            return;
        }
        this._handleConnect(chainId);
    ...
    _handleAccountsChanged(accounts, isEthAccounts = false) {
        ...
        if (!fast_deep_equal.default(this._state.accounts, _accounts)) {
